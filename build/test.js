...
let TestContractBase = (_dec = NearBindgen({}), _dec2 = view(), _dec(_class = (_class2 = class TestContractBase {
  nft_supply_for_owner({
    account_id
  }) {
    return account_id === "owner.test.near" ? 1 : 0;
  }
}, (_applyDecoratedDescriptor(_class2.prototype, "nft_supply_for_owner", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "nft_supply_for_owner"), _class2.prototype)), _class2)) || _class);
function nft_supply_for_owner() {
  const _state = TestContractBase._getState();
  if (!_state && TestContractBase._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = TestContractBase._create();
  if (_state) {
    TestContractBase._reconstruct(_contract, _state);
  }
  const _args = TestContractBase._getArgs();
  const _result = _contract.nft_supply_for_owner(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TestContractBase._serialize(_result, true));
}

export { nft_supply_for_owner };
